...
## 12. Phase H — Client Upload Front Door & Route Organization (🔴 REQUIRED — Sprint 5 is not usable without it)

> **Why this exists.** A post-merge PM review found the **headline value of Sprint 5 is unreachable by a real user**. The analyzer engine (A1/A2/B1), the results dashboard (C2/F1/F2), and chat (B2/C3) all exist — but **nothing in the UI lets a client feed a bill to the analyzer**:
> - **No frontend code calls `POST /api/client/energy-bills/upload`** — that AI route is referenced only by its own tests.
> - **There is no `<input type="file">` anywhere in the client app.** The only "Adicionar Fatura" form (`add-bill-form.tsx`) is the **Sprint-4 manual form** (type the numbers) that posts to the *manual* route and never touches the analyzer.
> - Because no bill ever goes through the analyzer from the UI, the rich dashboard has no AI-populated bill to render — so the user sees "nothing different".
>
> The original plan assumed an upload UI already existed ("B1 rewire the *existing* upload route"); it does not. Phase H builds the front door and nails down the route organization so the whole flow is reachable end-to-end.

### 12.1 Target route organization (implement + document)

**User-facing (client `@user`):**

| Route | Purpose | Entry from |
| --- | --- | --- |
| `/economia` | Economia home — bill list + Contas a Pagar. Two clearly-distinct entry actions: **"Adicionar fatura"** (manual number entry, existing) and **NEW "Analisar conta (PDF)"** (AI upload). | sidebar / cockpit |
| `/economia/[billId]` | **Unified bill analysis** — clarifier dashboard + technical-data viewer + chat drawer (F1/F2/C3). | bill-list rows **and auto-redirect after a successful AI upload** |

**API (already exist — Phase H only wires the UI to them; do not rebuild):**

| Endpoint | Role |
| --- | --- |
| `POST /api/client/energy-bills/upload` | **AI path** — multipart `{ file, consumerUnitId }` → `extract → analyze → persist` (B1) → returns `{ id, status }`. **← H1 calls this (currently orphaned).** |
| `POST /api/client/energy-bills` | Manual JSON entry (Sprint 4) — keep, it's the existing "Adicionar fatura". |
| `GET /api/economia/bills/[billId]` | Detail payload (C1). |
| `POST /api/economia/bills/[billId]/chat` | Streaming chat (B2). |

**Forbidden / removed:** `/economia/[billId]/analyze` was deleted in F1 — do **not** reintroduce a parallel analysis route. The single analysis surface is `/economia/[billId]`.

### 12.2 Task Table

| ID | Tier | Title | Owns |
| -- | ---- | ----- | ---- |
| H1 | L | Client AI-upload dialog (file → `/upload` → processing → redirect) | `src/frontend/economia/components/analyze-bill-dialog.tsx` (new) |
| H2 | S | Wire entry points + distinguish manual vs AI on `/economia` (+ cockpit checklist link) | `economia-screen.tsx` (modify), `onboarding-checklist.tsx` (modify, optional) |
| H3 | M | End-to-end user-path verification (the gate that was missing) | `analyze-bill-dialog` test + an economia-screen test |

**Order:** H1 → H2 → H3.

### 12.3 Briefs

#### H1 — Client AI-upload dialog · Tier L
**Goal:** a client picks a PDF/image of their bill, it runs through the analyzer, and they land on the analysis dashboard.

**Read first:** `src/frontend/economia/components/add-bill-form.tsx` (mirror its Dialog shell, UC `Select`, `useAuthenticatedApi`, toast/error patterns) and `src/app/api/client/energy-bills/upload/route.ts` (the contract: it reads `formData.get('file')` + `formData.get('consumerUnitId')`, returns `{ success, data: { id, status } }`).

**Owns (create):** `src/frontend/economia/components/analyze-bill-dialog.tsx`.

**Behavior:**
- A `Dialog` with: **Unidade consumidora** `Select` (client's UCs, fetched from `/client/consumer-units` like add-bill-form), a **file input** `accept="application/pdf,image/*"`, and a submit button.
- On submit: build `FormData` with `file` and `consumerUnitId`; `POST` to `/api/client/energy-bills/upload` (multipart — use `fetch`/the api client in a way that sends `FormData`, NOT JSON).
- **Four states:** idle; **processing** — the route runs two AI calls synchronously and can take 10–40s, so show a clear blocking state: `Analisando sua conta com IA… isso pode levar alguns segundos.` (spinner, disable inputs); **success** — `toast.success('Análise concluída!')`, close dialog, `router.push('/economia/' + data.id)`; **error** — `Alert variant="destructive"` with the server message.
- Validate a file is selected and a UC chosen before enabling submit.

**Copy (exact):** trigger button `Analisar conta (PDF)`; dialog title `Analisar conta de energia`; intro `Envie o PDF ou a foto da sua conta de luz. Nossa IA extrai os dados e gera a análise completa.`; file label `Arquivo da conta (PDF ou imagem)`; UC label `Unidade consumidora`; submit `Analisar conta`; processing `Analisando sua conta com IA… isso pode levar alguns segundos.`; success toast `Análise concluída!`.

**Note (flag, don't solve here):** the synchronous two-call latency risks a serverless/proxy timeout on large PDFs. Acceptable for v1 with the processing state; if real bills time out, the follow-up is an async job/queue (todo, out of H scope).

#### H2 — Entry points + manual/AI distinction · Tier S
- In `economia-screen.tsx`, mount `<AnalyzeBillDialog />` next to the existing `<AddBillForm />`, so `/economia` shows **both** actions with unambiguous labels: **"Adicionar fatura"** (manual) and **"Analisar conta (PDF)"** (AI). A one-line helper under them: `Tem o PDF da conta? Use "Analisar conta" para a IA preencher tudo.`
- (Optional) In the Controle cockpit onboarding checklist (`onboarding-checklist.tsx`), make the **"Primeira fatura"** row link to `/economia` so onboarding funnels here.
- After a successful upload, the user is already on `/economia/[billId]` (H1 redirect); ensure the bill list also refetches so the new bill shows when they go back.

#### H3 — End-to-end user-path verification · Tier M
**This is the gate the whole sprint lacked — verify the PATH, not just the pieces.**
- Component test for `analyze-bill-dialog.tsx`: selecting a file + UC and submitting POSTs **multipart** to `/api/client/energy-bills/upload` (mock fetch), shows the processing state, and on `{ success, data: { id } }` calls `router.push('/economia/'+id)`; on error shows the alert.
- `economia-screen` test: the screen renders **both** the "Adicionar fatura" and "Analisar conta (PDF)" entry points.
- Document a manual smoke-path in the handoff: `/economia` → "Analisar conta (PDF)" → pick a real bill PDF → processing → lands on `/economia/[billId]` → dashboard shows AI data → chat answers a question. (Requires deploy + `ANTHROPIC_API_KEY`.)

### 12.4 Phase H — Definition of Done
- [ ] **DoD-H1** — a client can upload a bill PDF/image from `/economia` and it runs through the analyzer (`/upload` reached from the UI for the first time).
- [ ] **DoD-H2** — after analysis the user lands on `/economia/[billId]` with the dashboard populated from AI data + working chat.
- [ ] **DoD-H3** — `/economia` clearly offers BOTH manual entry and AI upload, with distinct copy; bill list refetches.
- [ ] **DoD-H4** — the H3 tests pass; `tsc --noEmit` clean on H files; `vitest` green; `npm run build` clean.
- [ ] **DoD-H5** — the **full user path** (upload → processing → dashboard → chat) is verified, not just the components; smoke-path noted in the handoff.
- [ ] **DoD-H6** — deployed to the VPS with `ANTHROPIC_API_KEY` set (or `BILL_ANALYZER_PROVIDER=gemini`); confirmed working on one real bill. Until this is ticked, Sprint 5 is "done in code, not in production".

### 12.5 Phase H Ledger
Tick each H-task `[x]`, add a `Sprint 5 · H<n>` row to `billing.md`, post the §9-grade HANDOFF block (with the `tsc --noEmit` result and the end-to-end smoke-path note).

> **Lesson (add to the wall next to the Phase-G one):** a feature isn't "reachable" because a detail page renders — it's reachable when a user can **start** the flow. Always trace the path from the user's first click (here: "where do I upload my bill?"), not from the artifact backwards.

---

## 13. Phase I — Upload URL Fix (🔴 REQUIRED — the front door currently 404s)

> **Why this exists.** Phase H built a reachable, well-structured upload dialog, but the PM review found **one Critical bug that makes it fail in production**: the upload request goes to the wrong URL. The Phase-H tests passed because they assert the redirect/toast but **never assert the POST URL**, so the wrong URL slipped through green. Do **not** commit or deploy Phase H until Phase I closes.

### 13.1 The defect (with the exact fix)

| # | Sev | File:line | Problem | Exact fix |
|---|-----|-----------|---------|-----------|
| I-a | 🔴 | `src/frontend/economia/components/analyze-bill-dialog.tsx:106` | Posts to **`/api/client/energy-bills/upload`**, but `api.axiosInstance` has **`baseURL: '/api'`** (`src/frontend/auth/hooks/useAuth.ts:41`). The URL resolves to **`/api/api/client/energy-bills/upload` → 404**. (Every other call — incl. this file's own `/client/consumer-units` GET — omits the `/api` prefix.) User symptom: "Analisar conta" → spinner → generic error, never works. | Change the POST path to **`'/client/energy-bills/upload'`** (drop the leading `/api`). |
| I-b | 🟠 | `src/frontend/economia/components/analyze-bill-dialog.test.tsx` | The test mocks `axiosInstance.post` and asserts the redirect/toast but **never asserts the POST URL or that the body is `FormData`** — so a wrong URL (or a JSON body) passes green. This is the gap that hid I-a. | Add: `expect(mockAxiosPost).toHaveBeenCalledWith('/client/energy-bills/upload', expect.any(FormData))`. |

### 13.2 Task Table

| ID | Tier | Title | Owns |
| -- | ---- | ----- | ---- |
| I1 | S | Fix the upload URL + audit for sibling `/api` double-prefixes | `analyze-bill-dialog.tsx` |
| I2 | S | Add the URL/FormData assertion to the dialog test | `analyze-bill-dialog.test.tsx` |

**Order:** I1 · I2 (same feature, can be one commit).

### 13.3 Briefs

#### I1 — Fix the upload URL · Tier S
- Apply fix **I-a**.
- **Root-cause audit** (this is why it's not blind): grep the Phase-H surface for any other path that wrongly includes the `/api` prefix when called through `api`/`axiosInstance` (whose `baseURL` is already `/api`):
  ```bash
  grep -rnE "axiosInstance\.(post|get|put|patch|delete)\(['\"]/api/|api\.(post|get|put|patch|delete)\(['\"]/api/" src/frontend
  ```
  Fix every hit; report what you found (even if zero).

#### I2 — Guard the contract in the test · Tier S
- Apply fix **I-b**: assert the POST went to `'/client/energy-bills/upload'` with an `expect.any(FormData)` body. This is the assertion that would have caught I-a.

### 13.4 Verification gate (all must pass before handoff)
```bash
npx.cmd tsc --noEmit                                   # 0 errors in Phase H/I files
npx.cmd vitest run src/frontend/economia               # dialog + economia suite green, incl. the new URL assertion
npm.cmd run build
```

### 13.5 Phase I — Definition of Done
- [x] **DoD-I1** — the upload POSTs to `/client/energy-bills/upload` (resolves to the real `/api/client/energy-bills/upload`); the sibling-prefix audit is reported.
- [x] **DoD-I2** — the dialog test asserts the POST URL + `FormData` body and is green.
- [ ] **DoD-I3** — `tsc --noEmit` clean, `vitest` green, `npm run build` clean.
- [ ] **DoD-I4** — **end-to-end re-verified after deploy** (DoD-H6 still owns this): on the VPS with `ANTHROPIC_API_KEY` set, "Analisar conta (PDF)" with a real bill returns **200** (no 404), lands on `/economia/[billId]` with AI data, chat answers. Only then is the analyzer truly usable.

**Phase I execution note (2026-06-25):** I1/I2 implemented in the main workspace. Upload URL changed to `/client/energy-bills/upload`; sibling audit found and fixed four additional authenticated API calls in `src/frontend/plants/wizard/plant-wizard-screen.tsx` that were using `/api/client/...`. Post-fix audit returned zero matches. The dialog regression test now asserts `mockAxiosPost` was called with `/client/energy-bills/upload` and `expect.any(FormData)`.

**Verification evidence:** focused dialog test green (`6/6`); `npx.cmd vitest run src/frontend/economia` green (`48/48`). `npm.cmd run build` exited `0` outside the sandbox after the sandbox build worker crashed; it logged Prisma `P1001` because local Postgres was not running. `npx.cmd tsc --noEmit` is still blocked by unrelated existing type errors outside the Phase I files, so DoD-I3 remains open.

### 13.6 Phase I Ledger
Tick I1/I2 `[x]`, add `Sprint 5 · I1`/`I2` rows to `billing.md`, post the §9-grade HANDOFF block — and this time include the **POST-URL assertion** in the test evidence.

> **Lesson (third on the wall):** mock-based tests must assert the **contract** (URL, method, body shape), not just the side effects (redirect/toast). A mock that doesn't check what was sent will green-light a call to the wrong endpoint every time.
